...
The section below covers advanced configuration techniques for installing and using the AWS VPC Lattice Gateway API Controller. This includes things such as running the controller on a self-hosted cluster on AWS or using an IPv6 EKS cluster.

### Using a self-managed Kubernetes cluster

You can install the AWS VPC Lattice Gateway API Controller to a self-managed Kubernetes cluster in AWS.
