...
func Test_Delete_SnOwned_Success(t *testing.T) {
	...
	// No active service associations
	mockLattice.EXPECT().ListServiceNetworkServiceAssociationsAsList(ctx, gomock.Any()).Return(
		[]*vpclattice.ServiceNetworkServiceAssociationSummary{}, nil)
...
func Test_Delete_SnOwned_ActiveServiceAssociations(t *testing.T) {
	c := gomock.NewController(t)
	defer c.Finish()
	ctx := context.TODO()
	mockLattice := mocks.NewMockLattice(c)
	...
	err := snMgr.Delete(ctx, name)
	assert.NotNil(t, err)
	assert.Contains(t, err.Error(), "1 service association(s) still active")
}
