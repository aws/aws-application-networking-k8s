...
func (t *latticeTargetsModelBuildTask) buildModel(ctx context.Context) error {
	...
	if err != nil {
		t.log.Infof("Failed on buildLatticeTargets %s", err)
		return err
	}
...
func (t *latticeTargetsModelBuildTask) buildLatticeTargets(ctx context.Context) error {
	...
	if tg.ByServiceExport {
		...
		if err != nil {
			t.log.Infof("Failed to find Service export in the DS. Name:%s, Namespace:%s - err:%s", t.tgName, t.tgNamespace, err)
		} else {
			...
			for _, portAnnotation := range portsAnnotations {
				...
				if err != nil {
					t.log.Infof("Failed to read Annotations/Port:%s, err:%s", serviceExport.ObjectMeta.Annotations[portAnnotationsKey], err)
				} else {
					definedPorts[int32(definedPort)] = struct{}{}
				}
			}
			t.log.Infof("Build Targets - portAnnotations: %v", definedPorts)
		}
	...
	if svc.DeletionTimestamp.IsZero() {
		...
		t.log.Infof("Build Targets:  endpoints %s", endpoints)

		for _, endPoint := range endpoints.Subsets {
			for _, address := range endPoint.Addresses {
				for _, port := range endPoint.Ports {
					t.log.Infof("serviceReconcile-endpoints: address %v, port %v", address, port)
	...
	t.log.Infof("Build Targets--- targetIPList [%v]", targetList)
