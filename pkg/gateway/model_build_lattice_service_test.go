...
func Test_getACMCertArn(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                 "discovery disabled skips discovery",
			gw:                   httpsGateway,
			route:                makeRoute([]gwv1.Hostname{"app.example.com"}, nil),
			certDiscovery:        &stubCertDiscovery{arn: "should-not-be-used"},
		...
		{
			name:                 "discovery returns error logs warning and continues",
			gw:                   httpsGateway,
			route:                makeRoute([]gwv1.Hostname{"app.example.com"}, nil),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			origEnabled := config.CertDiscoveryEnabled
			config.CertDiscoveryEnabled = tt.certDiscoveryEnabled
			defer func() { config.CertDiscoveryEnabled = origEnabled }()
