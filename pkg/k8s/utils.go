...
const (
	...
	// Override lattice service generated by controller
	ServiceNameOverrideAnnotation = AnnotationPrefix + "service-name-override"

	// Service name annotation for ServiceExport mapping
	ServiceNameAnnotation = AnnotationPrefix + "service-name"
...
func GetServiceNameOverrideWithValidation(route metav1.Object) (string, error) {
	annotations := route.GetAnnotations()
	if annotations == nil {
		return "", nil
	}

	serviceNameOverride := annotations[ServiceNameOverrideAnnotation]
	if serviceNameOverride == "" {
		return "", nil
	}

	if err := ValidateVPCLatticeServiceName(serviceNameOverride); err != nil {
		return "", NewInvalidServiceNameOverrideError(serviceNameOverride, err.Error())
	}

	return serviceNameOverride, nil
}

func ValidateVPCLatticeServiceName(name string) error {
	if len(name) < 3 {
		return fmt.Errorf("service name must be at least 3 characters long")
	}
	if len(name) > 40 {
		return fmt.Errorf("service name cannot exceed 40 characters")
	}
	...
	if strings.HasPrefix(name, "svc-") {
		return fmt.Errorf("service name cannot start with 'svc-'")
	}

	if strings.HasPrefix(name, "-") {
		return fmt.Errorf("service name cannot start with a hyphen")
	}

	if strings.HasSuffix(name, "-") {
		return fmt.Errorf("service name cannot end with a hyphen")
	}

	if strings.Contains(name, "--") {
		return fmt.Errorf("service name cannot contain double hyphens")
	}

	return nil
}

type InvalidServiceNameOverride struct {
	ServiceName string
	Message     string
}

func (e *InvalidServiceNameOverride) Error() string {
	return fmt.Sprintf("invalid service name override '%s': %s", e.ServiceName, e.Message)
}

func NewInvalidServiceNameOverrideError(serviceName string, message string) error {
	return &InvalidServiceNameOverride{serviceName, message}
}

func IsInvalidServiceNameOverrideError(err error) bool {
	invalidServiceNameErr := &InvalidServiceNameOverride{}
	return errors.As(err, &invalidServiceNameErr)
}

// GetServiceNameFromServiceExport returns the service-name annotation value if set,
// otherwise falls back to the ServiceExport resource name.
func GetServiceNameFromServiceExport(serviceExport client.Object) string {
	if serviceExport == nil {
		return ""
	}
	if serviceExport.GetAnnotations() != nil {
		if serviceName, exists := serviceExport.GetAnnotations()[ServiceNameAnnotation]; exists {
			if trimmed := strings.TrimSpace(serviceName); trimmed != "" {
				return trimmed
			}
		}
	}
	return serviceExport.GetName()
}

// GetExportNameFromServiceImport returns the export-name annotation value if set, or "" if not.
func GetExportNameFromServiceImport(serviceImport client.Object) string {
	if serviceImport != nil && serviceImport.GetAnnotations() != nil {
		if exportName, exists := serviceImport.GetAnnotations()[ExportNameAnnotation]; exists {
			if trimmed := strings.TrimSpace(exportName); trimmed != "" {
				return trimmed
			}
		}
	}
